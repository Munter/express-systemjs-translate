...
module.exports = function (options) {
  if (typeof options !== 'object') {
    throw new Error('express-systemjs-translate: Missing options. A configuration object must be passed as the first argument');
  }

  if (typeof options.serverRoot !== 'string') {
    throw new Error('express-systemjs-translate: Missing option: serverRoot');
  }
  ...
  options = extend({
    baseUrl: options.serverRoot,
    configFile: Path.join(options.serverRoot, 'system.config.js'),
    bundle: true,
    watchFiles: true,
    debug: false,
    depCache: false,
  ...
  var serverRoot = Path.resolve(process.cwd(), options.serverRoot);
  var baseURLPath = fromFileURL(builder.loader.baseURL);
  var rootRelativeConfigUrl = Path.relative(serverRoot, absoluteConfigUrl);
  var rootRelativeBaseUrlPath = Path.relative(serverRoot, baseURLPath);
  ...
  function upgradeSourceMaps(options, result) {
    ...
    if (result.sourceMap) {
      ...
      if (!map.sourcesContent) {
        ...
        if (result.tree && result.tree[map.file]) {
          map.sourcesContent.push(result.tree[map.file].source);
        }
      }
  ...
  return function (req, res, next) {
    ...
    if (accept && accept.indexOf('application/x-es-module') !== -1) {
      var fileName = Path.join(serverRoot, req.path);
