...
var wrapLeaky = require('./wrapLeaky');
...
module.exports = function systemJsTranslate(serverRoot, options) {
  ...
  try {
    wrapLeaky(function () {
      ...
      Builder = parentRequire('jspm').Builder;
    ...
    try {
      // Will leak globals: __curScript, URLPolyfill, SystemJS, System
      wrapLeaky(function () {
  ...
  return function (req, res, next) {
    ...
    if (accept && accept.indexOf('application/x-es-module') !== -1) {
      ...
      if (options.bundle) {
        builderPromise = wrapLeaky(function () {
          return builder.bundle(fileName, extend(options.buildFlags, { config: { browserRootURL: req.protocol + '://' + req.headers.host } }));
        });
      } else {
        // If source maps are wanted, also put in source files content
        builderPromise = wrapLeaky(function () {
          return builder.compile(fileName, extend(options.buildFlags, { sourceMapContents: !!options.sourceMap, config: { browserRootURL: req.protocol + '://' + req.headers.host } }));
        });
      }
