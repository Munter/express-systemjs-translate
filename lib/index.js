...
module.exports = function systemJsTranslate(serverRoot, options) {
  ...
  try {
    ...
    Builder = require.main.require('jspm').Builder;

    builder = new Builder();

    absoluteConfigUrl = require.main.require('jspm/lib/config').pjson.configFile;

    hasJspm = true;
  } catch (err) {
    // JSPM not installed. Lets try systemjs-builder
    try {
      Builder = require.main.require('systemjs-builder');
